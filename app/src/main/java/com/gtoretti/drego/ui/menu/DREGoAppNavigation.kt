...
class DREGoNavigationActions(navController: NavHostController) {
    ...
    val navigateToAccountingAccounts: () -> Unit = {
        navController.navigate(DREGoDestinations.ACCOUNTING_ACCOUNTS) {
            popUpTo(navController.graph.findStartDestination().id) {
                saveState = true
            }
            launchSingleTop = true
    ...
    val navigateToBalanceSheet: () -> Unit = {
        navController.navigate(DREGoDestinations.BALANCE_SHEET) {
            popUpTo(navController.graph.findStartDestination().id) {
                saveState = true
            }
            launchSingleTop = true
    ...
    val navigateToEquityChangesStatement: () -> Unit = {
        navController.navigate(DREGoDestinations.EQUITY_CHANGES_STATEMENT) {
            popUpTo(navController.graph.findStartDestination().id) {
                saveState = true
            }
            launchSingleTop = true
    ...
    val navigateToCashFlowsStatement: () -> Unit = {
        navController.navigate(DREGoDestinations.CASH_FLOWS_STATEMENT) {
            popUpTo(navController.graph.findStartDestination().id) {
                saveState = true
            }
            launchSingleTop = true
    ...
    val navigateToPeriodResultsStatement: () -> Unit = {
        navController.navigate(DREGoDestinations.PERIOD_RESULTS_STATEMENT) {
            popUpTo(navController.graph.findStartDestination().id) {
                saveState = true
            }
            launchSingleTop = true
    ...
    val navigateToExplanatoryNotes: () -> Unit = {
        navController.navigate(DREGoDestinations.EXPLANATORY_NOTES) {
            popUpTo(navController.graph.findStartDestination().id) {
                saveState = true
            }
            launchSingleTop = true
